...
final class ControlEndpointService extends ServiceLooper
{
	...
	boolean handleServiceType(final KNXnetIPHeader h, final byte[] data, final int offset, final InetSocketAddress src)
		...
		logger.trace("{} received {} {}", svcCont.getName(), hostPort(src),
		...
		if (h.isSecure()) {
			...
			if (inShutdown) {
				logger.trace("{} is being shut down, ignore connect request from {}",
	...
	boolean acceptControlService(final int sessionId, final KNXnetIPHeader h, final byte[] data, final int offset,
		...
		if (svc == KNXnetIPHeader.SearchRequest) {
			...
			logger.debug("received connect response - ignored");
			...
			if (conn == null) {
				logger.debug("received disconnect request with unknown channel id " + dr.getChannelID() + " - ignored");
				return true;
			}
			...
			if (endpoint == null)
				...
				if (status == ErrorCodes.NO_ERROR) {
					logger.trace("received connection-state request (channel {}) from {}",
			...
			logger.debug("received connection-state response - ignored");
		else {
		...
		try (final var scope = new TaskScope("data connection closer", Duration.ofSeconds(12))) {
			for (final DataEndpoint endpoint : connections.values())
				scope.execute(() -> endpoint.close(CloseEvent.SERVER_REQUEST,
						"quit service container " + svcCont.getName(), LogLevel.INFO, null));
		}
	}
