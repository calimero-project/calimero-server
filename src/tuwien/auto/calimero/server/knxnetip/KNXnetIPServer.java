	...
	*/
	...
	synchronized String getOption(final String optionKey)
	{
		...
		if (OPTION_DISCOVERY_INTERFACES.equals(optionKey)) {
			return join(discoveryIfs, ",");
		}
		if (OPTION_OUTGOING_INTERFACE.equals(optionKey)) {
			return join(outgoingIf, ",");
		}
	...
	private NetworkInterface[] parseNetworkInterfaces(final String optionKey, final String value)
	{
		...
		return (NetworkInterface[]) l.toArray(new NetworkInterface[l.size()]);
	}
	...
	private static String join(final NetworkInterface[] array, final String sep)
	{
		if (array == null)
			return "default";
		final StringBuffer sb = new StringBuffer();
		for (int i = 0; i < array.length; i++) {
			final NetworkInterface ni = array[i];
			sb.append(ni.getName()).append(sep);
		}
		return sb.toString();
	}
		...
		final boolean routingSupported)
	{
		...
		try {
			...
			if (routingSupported && device.getRawAddress() == 0)
				...
				ios.setProperty(knxObject, objectInstance, PID.KNX_INDIVIDUAL_ADDRESS, 1, 1,
						((ServiceContainer) svcContainers.get(svcContainers.size() - 1)).getSubnetAddress()
								.toByteArray());
		}
		catch (final KNXPropertyException e) {
...
//			}
		...
		private void sendOnInterfaces(final DatagramPacket p) throws SocketException, IOException
		{
			...
			final List nifs = outgoing.length > 0 ? Arrays.asList(outgoing)
					: Collections.list(NetworkInterface.getNetworkInterfaces());
			for (final Iterator i = nifs.iterator(); i.hasNext();) {
				final NetworkInterface nif = (NetworkInterface) i.next();
