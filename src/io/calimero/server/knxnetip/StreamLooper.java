...
abstract sealed class StreamLooper implements Runnable, AutoCloseable permits UnixDomainSocketEndpoint.Looper {
	...
	StreamLooper(final ControlEndpointService ces, final EndpointAddress endpoint, final boolean baos) {
		this.ces = ces;
	...
	boolean inactive() { return !ces.sessions.anyMatch(endpoint) && !ces.anyMatchDataConnection(endpoint); }

	abstract int read(final byte[] data, final int offset) throws IOException;
